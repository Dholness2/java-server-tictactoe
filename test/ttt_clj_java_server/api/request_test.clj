...
(deftest get-full-message-test
  (testing "it returns full request message "
    (let [request (Request.)
          expected  "GET /next_move?openPosition=0&move=Submit HTTP/1.1"]
      (.setMessage request "GET /next_move?openPosition=0&move=Submit HTTP/1.1")
      (is (= expected (get-full-message request))))))
